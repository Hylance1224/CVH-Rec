...
if __name__ == '__main__':
    ...
    # 加载 API ID -> 名称 映射
    with open("Original Dataset/new_apis.json", "r", encoding="utf-8") as f:
        api_data = json.load(f)
        api_id_to_title = {item["id"]: item["url"] for item in api_data}
        all_api_ids = set(api_id_to_title.keys())

    # --- 统计调用历史：mashup.json中前80%记录 ---
    with open(f"Original Dataset/new_mashups.json", "r", encoding="utf-8") as f:
    ...
    recommendations = {}
    with open(
            f"output/R2API_5_0.6_0.6_0.8_0.8_fold.json",
            "r") as f:
        for line in f:
    ...
    print(f"The performance of {fold} on specified mashup_ids are as follows:")
    for N in Ns:
        precision_list = []
        recall_list = []
        map_list = []
        ...
        for i, (mashup_id, true_apis) in enumerate(ground_truth.items()):
            ...
            if total_interactions > 0:
                pb_value = sum(hit_counts.get(api_id, 0) / total_interactions for api_id in recommended_apis) / N
            else:
                pb_value = 0
        ...
        print(f"N={N} -> Precision: {average_precision:.4f}, Recall: {average_recall:.4f}, "
              f"MAP: {average_map:.4f}, NDCG: {average_ndcg:.4f}, Coverage: {coverage:.4f}, PB: {average_pb:.4f}")
