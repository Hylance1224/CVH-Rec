...
def save_test_results_to_json(mashup_name, true_apis, predicted_apis, filename='test_results.json'):
    result = {
...
def decode_one_batch_rl(greedy, hidden_n, outputs, outputs_category, src_lens, category_lens, tgt, tgt_lens):
    ...
    for di in range(predict_api_num):
        y_t_1 = y_t
...
def infer_one_batch_ml(
    ...
    predictions = torch.full([10, src_lens.shape[0], api_size], 0, dtype=torch.float32).cuda() #这个10不能改

    step_losses = []
    for di in range(10): #这个10不能改
        if di == 0:
            y_t_1 = tgt[di]
        else:
            y_t_1 = y_t
...
def decode_greedy(
    ...
    file = open(f'decode_greedy{epoch}.jsonl', 'a')
    decode_seq = []
    for di in range(100):
        if di == 0:
            y_t_1 = tgt[di]
        ...
        mul_ = (ones - outputs_retrive2).repeat(api_size, 1).permute(1, 0).unsqueeze(0).cuda()
        if use_retrieval:
        ...
        if di == 0:
            topk = torch.topk(prediction, predict_api_num, 2)
            removed_apis = tgt.permute(1, 0)[:, 1:].cpu().numpy().tolist()  # shape: (batch_size, tgt_len)
            predicted_apis = topk.indices.squeeze(0).cpu().numpy().tolist()  # shape: (batch_size, predict_api_num)

            for i in range(len(mashup_names)):
                record = {
                    "mashup_name": mashup_names[i],
    ...
    with tqdm(enumerate(test_iter), total=len(test_iter)) as loop:
        ...
        for batch_id, batch_data in loop:
            ...
            decode_list, ndcg__, recall__, ap__, pre__ = decode_greedy(hidden_n, outputs, outputs_category, src_lens,
                                                                       ...
                                                                       retrieve_category_lens, tgt_apis, mashup_names, epoch)
            ...
            # for idx, mashup_name in enumerate(mashup_names):
            #     predicted_apis = all_mashup_services[idx]
            #     true_apis = tgt_apis[idx]
            #
            #     # 打印 mashup 名称, 真实API和预测API
            #     print(f"Mashup: {mashup_name}")
            #     print(f"True APIs: {true_apis}")
            #     print(f"Predicted APIs: {decode_list[idx]}")
            #     # print('-' * 50)
