...
class APIDataset(Dataset):
    def __init__(self, data_file, rfile, rfile2,max_vocab_size=3000, train_dataset=None, word_dim=300, oov=None):
        ...
        self.data_file = data_file  # 传入的是X_train，即需要训练的数据
        print('self.data_file:', len(self.data_file))
        print('1'*40)
        ...
        # self.src是训练mashup的描述,self.retrieves是检索mashup的id
        print("len(self.src)", len(self.src))
        # print("self.retrieves:", self.retrieves)
        # print("self.retrieves:", self.retrieves)
        # print('========')
        # sys.exit()
        ...
        if train_dataset is None:
            ...
            Label = data.Field(sequential=True, init_token=SOS, eos_token=EOS, pad_token=PAD)
    ...
    #     new_tokens = []
    #     if append_BOS: new_tokens.append(SOS)
    #     new_tokens.extend([t for t in tokens if t in token2id])  # 过滤
    #     if append_EOS: new_tokens.append(EOS)
    #     seq = [token2id[token] for token in new_tokens]
    #     return seq
    ...
    def load_sents2(self,  rfile, rfile2, oov):
        ...
        with open(rootPath + '/data/mashup_name.json', 'r', encoding='utf-8') as f:
            mashup_names_ = json.load(f)  # 所有mashup的name
        ...
        if oov is not None:
            ...
            with open(rootPath + '/data/api_name.json', 'r', encoding='utf-8') as f, open(rootPath + '/data/api_description.json', 'r', encoding='utf-8') as f2, open(rootPath + '/data/api_category.json', 'r', encoding='utf-8') as f3:
                apis = json.load(f)
                descs = json.load(f2)
                cats = json.load(f3)
        ...
        for idx, mashup in enumerate(mashup_names_): # mashup_names_是所有mashup的name
