...
from src.datamodules.components.mashup_dataset_new import MashupDataset
...
class MashupDataModule(LightningDataModule):
    ...
    def __init__(
        ...
        mashup_index: bool = True, # need to change
        api_index: bool = False,
        is_orderly: bool = True,  #delete
    ):
    ...
    def setup(self, stage: Optional[str] = None) -> None:
        ...
        if self.is_orderly:
            train_idx = list(range(self.train_val_test_split[0]))
            test_idx = list(range(self.train_val_test_split[0], self.train_val_test_split[0] + self.train_val_test_split[1]))
            val_idx = list(range(self.train_val_test_split[0] + self.train_val_test_split[1],self.train_val_test_split[0] + self.train_val_test_split[1] +self.train_val_test_split[2]))

        else:
            train_idx, test_idx, val_idx = random_split(range(num_mashups), self.train_val_test_split)
    ...
    def train_dataloader(self) -> TRAIN_DATALOADERS:
        dataset = MashupDataset(
            ...
            mashup_index=self.mashup_index,
