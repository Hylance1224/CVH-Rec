...
class GSL_MASH(LightningModule):
    ...
    def __init__(
        ...
        edge_message = True,
        num_candidates = 1382   #932 or 21495
    ):
        r"""A MLP with two linear layers.
        ...
        self.recall = torchmetrics.Recall(top_k=5, task="multilabel", num_labels=self.num_api)
        ...
        self.mashup_path = "data/api_mashup/embeddings/text_bert_mashup_embeddings.npy"
    ...
    def training_step(self, batch: Any) -> STEP_OUTPUT:
        ...
        # logging.info(f"1input_feature.shape: {input_feature.shape}")
        # logging.info(f"batch_size: {batch_size}")
        # logging.info(f"num_candidates: {self.num_candidates}")
    ...
    def validation_step(self, batch: Any, batch_idx: int) -> Optional[STEP_OUTPUT]:
        ...
        # logging.info(f"2input_feature.shape: {input_feature.shape}")
        # logging.info(f"batch_size: {batch_size}")
        # logging.info(f"num_candidates: {self.num_candidates}")
        ...
        # self.log('val/Recall@5', self.recall(preds, labels), on_step=False, on_epoch=True, prog_bar=True)
    ...
    def test_step(self, batch: Any, batch_idx: int) -> Optional[STEP_OUTPUT]:
        ...
        # logging.info(f"3input_feature.shape: {input_feature.shape}")
        # logging.info(f"batch_size: {batch_size}")
        # logging.info(f"num_candidates: {self.num_candidates}")
        ...
        # 下面是保存id, preds和targets到json文件
        # 注意先把tensor转成list，方便json序列化
        mashup_ids = mashups_idx.cpu().tolist()
        preds_list = preds.detach().cpu().tolist()
        targets_list = labels.cpu().tolist()

        # 逐条写入json文件（追加模式）
        topk_indices = torch.topk(preds, k=20, dim=1).indices.cpu().tolist()  # Top-10 推荐的 API id
        targets_id_list = [ [j for j, val in enumerate(targets) if val == 1] for targets in targets_list ]  # 提取1所在的下标
        with open('GSL-Mash.json', 'a', encoding='utf-8') as f:
            for i in range(batch_size):
                record = {
                    "mashup_id": mashup_ids[i],
                    "recommend_api": topk_indices[i],
                    "remove_apis": targets_id_list[i]
                }
