...
#                               hidden_size=128,
#                               num_api=1382, num_tags=409, num_epochs=100, output_file="output_results_10_fold.txt"):
...
# 改成前7折为训练集，中间2折为测试集（不再十次循环，而是一次划分一次训练）
def run_comparison_experiment(descriptions_file, api_calls_file, labels_file, folds_file, input_size=384,
                              hidden_size=128,
                              num_api=1382, num_tags=409, num_epochs=400, output_file="output_results_9_fold_split.txt"):
    descriptions = read_mashup_descriptions(descriptions_file)
    labels = read_labels(labels_file)
    api_calls = read_api_calls(api_calls_file)
    fold_ids = read_fold_ids(folds_file)  # 读取十折列表（列表中包含十个列表，每个列表是一折的id）
    ...
    # 手动划分 0~6 折为训练，7~8折为测试，剩下1折不使用
    train_ids = [idx for fold in fold_ids[:7] for idx in fold]
    val_ids = [idx for fold in fold_ids[7:9] for idx in fold]

    with open(output_file, 'w', encoding='GBK') as f:
        f.write("Training and evaluation results:\n")
        f.write("使用前7折为训练集，8-9折为测试集\n")
        ...
        optimizer = optim.Adam(model.parameters(), lr=0.01)
        for epoch in range(num_epochs):
            train_data, train_api_labels, train_tag_labels = shuffle_data(train_data, train_api_labels, train_tag_labels)
            train_loss, api_loss, tag_loss = train_model(optimizer, model, train_data, train_api_labels, train_tag_labels)
            f.write(f"Epoch {epoch + 1} - 训练损失：{train_loss}\n")
            f.write(f"api损失：{api_loss}，tag损失：{tag_loss}\n")
        ...
        # 保存推荐结果到JSON文件
        recommend_results = []
        predicted_apis = torch.argsort(api_output, dim=-1, descending=True)[:, :20].cpu().numpy()

        for idx, mashup_id in enumerate(val_ids):
            result = {
                "mashup_id": mashup_id,
                "recommend_api": predicted_apis[idx].tolist(),
                "api_target": api_calls.get(mashup_id, [])
            }
            recommend_results.append(result)

        with open(f"./output/recommend_DNN_split.json", 'w', encoding='utf-8') as file:
            for result in recommend_results:
                json.dump(result, file, ensure_ascii=False)
                file.write("\n")
