...
if __name__ == '__main__':
    ...
    # 加载 API ID -> 名称 映射
    with open("Original Data/new_apis.json", "r", encoding="utf-8") as f:
        api_data = json.load(f)
        api_id_to_title = {item["id"]: item["url"] for item in api_data}
        all_api_ids = set(api_id_to_title.keys())

    # --- 统计调用历史：mashup.json中前80%记录 ---
    with open(f"Original data/new_mashups.json", "r", encoding="utf-8") as f:
    ...
    with open(f"{method}.json", "r") as f:
        for line in f:
            ...
            remove_apis = data["api_target"]  # 这是 ground truth
    ...
    for N in Ns:
        ...
        for i, (mashup_id, true_apis) in enumerate(ground_truth.items()):
            ...
            recall = hit_count / len(list(true_set))
